...
class RepairCaseEquipment(Base):
    ...
    regional_center_id: Mapped[int] = mapped_column(
        ForeignKey("regional_centers.id"), nullable=False
    )
    locomotive_model_id: Mapped[int] = mapped_column(
        ForeignKey("locomotive_models.id"), nullable=False
    )
    fault_discovered_at_id: Mapped[int] = mapped_column(
        ForeignKey("fault_discovery_places.id"), nullable=False
    )
    component_equipment_id: Mapped[int] = mapped_column(
    ...
    performed_by_id: Mapped[int | None] = mapped_column(
        ForeignKey("repair_performers.id")
    )
    equipment_owner_id: Mapped[int | None] = mapped_column(
        ForeignKey("equipment_owners.id")
    )
    ...
    locomotive_model: Mapped["LocomotiveModel"] = relationship(
        "LocomotiveModel", back_populates="repair_cases"
    )
    fault_discovered_at: Mapped["FaultDiscoveryPlace"] = relationship(
        "FaultDiscoveryPlace", back_populates="repair_cases"
    )
    ...
    performed_by: Mapped["RepairPerformer | None"] = relationship(
        "RepairPerformer", back_populates="repair_cases"
    )
    equipment_owner: Mapped["EquipmentOwner | None"] = relationship(
        "EquipmentOwner", back_populates="repair_cases"
    )
